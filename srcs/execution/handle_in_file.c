		...
		t_create_heredoc *data)
{
	...
	free(data->heredoc_file);
	exec->fd_infile[data->i] = data->file_in;
	here_doc(exec->infile_name[data->i][data->j], data->file_in, dictionary,
		exec);
	if (g_signal == 130)
	{
		close(data->file_in);
		...
		t_create_heredoc *data)
{
	...
	{
		result = process_here_doc(exec, dictionary, data);
		if (result == 2)
			return (result);
		else if (result == 1)
			return (result);
...
void	handle_in_file(t_execution *exec, t_dict *dictionary)
{
	...
	while (exec->infile_name[data.i] != NULL)
	{
		...
		{
			result = handle_in_file2(exec, dictionary, &data);
			if (result == 2)
				continue ;
			else if (result == 1)
				return ;
